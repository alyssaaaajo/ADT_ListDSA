...
void addChild(Tree *T, Node parent_index, Node child_index) {
    if (parent_index < 0 || parent_index >= MAX_NODES) return;
...
Node left(const Tree *T) {
    for (int i = 0; i < MAX_NODES; i++) {
        // Skip if node is not part of tree
        if (i == T->rootIndex) {} // Root can be leaf if no children
        else if (parent(i, T) == INVALID_NODE) continue;

        if (T->children[i] == NULL) return i; // no children → leaf
    }
    return INVALID_NODE;
}
...
int main() {
    ...
    if (p == ROOT_NODE) printf("Parent of Node %d: (It is the Root)\n", node_to_check);
    else if (p == INVALID_NODE) printf("Parent of Node %d: (Node is Invalid/Not Found)\n", node_to_check);
    else printf("Parent of Node %d: Node %d\n", node_to_check, p);
    ...
    node_to_check = 7;
    p = parent(node_to_check, &myTree);
    if (p == INVALID_NODE) printf("Parent of Node %d: (Node is Invalid/Not Found)\n", node_to_check);
    else printf("Parent of Node %d: Node %d\n", node_to_check, p);

    Node l = left(&myTree);
    if (l != INVALID_NODE) printf("Leftmost Leaf: Node %d\n", l);
    else printf("Leftmost Leaf: Tree is empty or has no leaves.\n");

    Node r = right(&myTree);
    if (r != INVALID_NODE) printf("Rightmost Leaf: Node %d\n", r);
    else printf("Rightmost Leaf: Tree is empty or has no leaves.\n");
